...
class ModQADataset(Dataset):
    def __init__(
        self,
        data_dir: str = "/scratch/jl13122/zsre_modified_datasets",
            ...
            data.append(
                {
                    ...
                    "requested_rewrite": {
                        "prompt": self._clean_and_replace(
                            record["prompt_for_object"][0], 
                            subject, 
                            "{}",
                            second_only=True
                        ),
                        "subject": record["subject"],
                        "target_new": {"str": self._clean(record["object_false"]), "id": record["object_id"]},
                        "target_true": {"str": self._clean(record["object_true"]), "id": record["object_id"]},
                    ...
                    "paraphrase_prompts": [self._clean(p) for p in (record.get("prompt_for_object", [])[:2])],
                    ...
                    "generation_prompts": [self._clean(p) for p in record.get("prompt_for_object", [])[2:]],
    ...
    def _clean(text):
        ...
        """
        allowed_chars = set(",.+-|\\/abÉcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 ?")
