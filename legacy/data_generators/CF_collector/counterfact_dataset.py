...
def generate_answer(prompt_text, retries=3, delay=5):
    for attempt in range(retries):
        try:
            response = openai.ChatCompletion.create(
                model="gpt-3.5-turbo",
...
for item in tqdm(selected_frequent + selected_infrequent, desc="Processing selected entries"):
    ...
    prompt_for_relation = f"Given the subject '{subject}', relation '{relation}', and an object '{object_false}', generate a question that implies the subject's correct relationship to a object, without using the exact term '{relation}'. The goal is to craft a question that tests knowledge of this relationship in a natural way. For example, if given 'subject': 'Danielle Darrieux', 'relation': 'native language',  'object': 'English', generate questions like 'what does English mean to Danielle Darrieux?', usually, your qeustion should at least look like 'What is the relationship between '{subject}' and '{object_false}'? Don't generate question that leads to answers of the suject or the object itself. The answer of your geenrated answer should be the relation: '{relation}'."
    ...
    attribute_question_subject1 = generate_questions(f"Generate a question for any attribute of {subject}. For example, if the subject is a person, you can ask'Where does that person come from?'")
    attribute_question_object1 = generate_questions(f"Generate a question for any attribute of {object_false}. For example, if the subject is a person, you can ask'Where does that person come from?'")
    attribute_question_subject2 = generate_questions(f"Generate a question for any attribute of {subject}. For example, if the subject is a person, you can ask'Where does that person come from?'")
    attribute_question_object2 = generate_questions(f"Generate a question for any attribute of {object_false}. For example, if the subject is a person, you can ask'Where does that person come from?'")
    ...
    entry = {
        ...
        "attribute_question_subject1": attribute_question_subject1,
        "attribute_question_subject_answer1": generate_answer(f"Give the answer to this question, make sure you search for enough resources and the answer you provide is absolutely correct: {attribute_question_subject1}"),
        "attribute_question_object1": attribute_question_object1,
        "attribute_question_obejct_answer1": generate_answer(f"Give the answer to this question, make sure you search for enough resources and the answer you provide is absolutely correct: {attribute_question_object1}"),
        "attribute_question_subject2": attribute_question_subject2,
        "attribute_question_subject_answer2": generate_answer(f"Give the answer to this question, make sure you search for enough resources and the answer you provide is absolutely correct: {attribute_question_subject2}"),
        "attribute_question_object2": attribute_question_object2,
        "attribute_question_obejct_answer2": generate_answer(f"Give the answer to this question, make sure you search for enough resources and the answer you provide is absolutely correct: {attribute_question_object2}"),
...
# Output path
output_path = "generated_dataset/counterfact_dataset.json"
